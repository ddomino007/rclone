...
func (fs *FileService) CreateFile(c *gin.Context) (*schemas.FileOut, *types.AppError) {
	...
	if err := fs.Db.Create(&fileDB).Error; err != nil {
		...
		if pgErr.Code == "23505" {
			fs.log.Warn("File already exists, removing...", zap.String("fileName", fileDB.Name))

			if err := fs.Db.Where("name = ?", fileDB.Name).Delete(&models.File{}).Error; err != nil {
				return nil, fs.logAndReturn("file remove", err, http.StatusInternalServerError)
			}
